...
public class InitializeMR extends Configured implements Tool{
	@Override
	public int run(String[] arg0) throws Exception {
	    @SuppressWarnings("deprecation")
		Job job=new Job();
	    
	    job.setJobName("Initialize Map Reduce");
	    job.setJarByClass(InitializeMR.class);
	    
	    //set map reduce class
	    job.setMapperClass(InitializeMRMapper.class);
	    job.setReducerClass(InitializeMRReducer.class);
	...
	public static class InitializeMRMapper extends Mapper<LongWritable,Text,Text,Text> {
		
		final String[] stopWords = { "a", "able", "about", "above", "abst", "accordance", "according", "accordingly",
		...
		public void map(LongWritable key ,Text values,Context context) throws IOException, InterruptedException {
			...
			//remove tags and specific sections of the html
			String content = parts[1];
			String content_tagless = content.replaceAll("<head>(.*?)</head>"     , " ")
											.replaceAll("<script>(.*?)</script>" , " ")
											.replaceAll("<[^>]+>"                , " ")
											.trim();
			
			//split by spaces
			ArrayList<String> split_content = new ArrayList<String>();
		    String[] split_arr = content_tagless.split(" ");
		    for (String s : split_arr) { split_content.add(s); }
		    
		    //add the host's name to the indexable words (twitter, google, etc...)
		    String host = parts[0].replaceAll("http://", "")
		    					  .replaceAll("https://", "")
		    					  .replaceAll(".com", "")
		    					  .replaceAll("www.", "").split("/")[0];
		    split_content.add(host);
		    
		    for (String s : split_content) {
		    	if (!s.equals("")) {
		    		...
		    		for (String stop : stopWords) {
		    			if (s_lower.equals(stop)) { stopped = true; break; }
		    		}
		    		if (stopped) { continue; }
		    		...
		    		for (char c : s_lower.toCharArray()) {
		    			if (!Character.isLetter(c)) { special = true; break; }
		    		}
		    		if (special) { continue; }
		    		
		    		//STEMMER
		    		Stemmer st = new Stemmer();
		    		for (char ch : s_lower.toCharArray()){ st.add(ch); }
		    		st.stem(); String s_stem = st.toString();
		    		
		    		//write ((WORD : DOCNAME), ("1"))
		    		context.write(new Text(s_stem + " : " + parts[0]), new Text("1"));
		    	}
		    }
		}
	}

	public static class InitializeMRReducer extends Reducer<Text,Text,Text,Text> {
		@Override 
		public void reduce(Text key ,Iterable<Text> values,Context context) throws IOException, InterruptedException {
			//counts times that this word appears in the DOC
			String k       = key.toString();
			String[] split = k.split(" : ", 2);
			
			double freq = 0;
			for (@SuppressWarnings("unused") Text t : values) { freq++; }
			
			//write ((WORD), (DOCNAME : FREQUENCY))
			context.write(new Text(split[0]), new Text(split[1] + " : " + Double.toString(freq)));
		}	
	}
